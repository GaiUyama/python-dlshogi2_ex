...
hcpe_file_list = glob.glob(os.path.join(args.hcpe_dir, '**', '*.hcpe'), recursive=True)

# ファイルリストをシャッフル
# file_list_train, file_list_test = train_test_split(csa_file_list, test_size=args.test_ratio)
file_list_train, file_list_test = train_test_split(hcpe_file_list, test_size=args.test_ratio)
